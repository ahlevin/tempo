...
export default function HomeScreen() {
  ...
  function urgColor(d: number) {
    return d <= 7 ? '#E8507A' : d <= 30 ? '#F0A04B' : '#3ECFB2';
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.bg }} edges={['top']}>
      ...
      >
        ...
        {filteredEvents.length === 0 && filter !== 'goal' ? (
          ...
          return (
            <TouchableOpacity key={e.id} activeOpacity={0.8}
              onLongPress={() => router.push({ pathname: '/modals/edit-event', params: { id: e.id } })}
        ...
        {showGoals && <>
          ...
          {goals.length === 0 ? <EmptyState text="No goals yet. Tap + Add to create one." /> :
            goals.map(g => {
              ...
              return (
                <View key={g.id} style={{ backgroundColor:Colors.surf, borderRadius:18,
                  ...
                  <View style={{ flexDirection:'row', alignItems:'center', gap:8 }}>
                    ...
                    <Text style={{ fontSize:11, fontWeight:'700', color:Colors.teal }}>{pct}%</Text>
                    {!done && <View style={{ flexDirection:'row', gap:5 }}>
          ...
          return (
            <View style={{ gap:8 }}>
              {[
                ...
                const dt = days - s.thr;
                const badge = dt < 0 ? '✓ Passed' : dt <= 10 ? 'In ' + dt + 'd' : 'In ' + dt + 'd';
                const color = dt < 0 ? Colors.rose : dt <= 10 ? Colors.amber : Colors.teal;
                const bg    = dt < 0 ? 'rgba(232,80,122,0.15)' : dt <= 10 ? 'rgba(240,160,75,0.12)' : 'rgba(62,207,178,0.1)';
                return (
                  <View key={i} style={{ flexDirection:'row', alignItems:'center', gap:11,
                    ...
                    <View style={{ flex:1 }}>
                      ...
                      <Text style={{ fontSize:11, color:Colors.text3 }}>for {soonest.emoji} {soonest.name}</Text>
                    </View>
        ...
        {memories.length === 0 ? <EmptyState text="No memories yet. Tap + Add to create one." /> :
          memories.map(m => {
            ...
            const years = now.getFullYear() - o.getFullYear();
            const count = m.entries.length;
            const display = m.type === 'lifelog' ? count + (count === 1 ? ' time' : ' times') : years + ' years';
            return (
              <TouchableOpacity key={m.id} activeOpacity={0.8}
                ...
                <View style={{ flexDirection:'row', alignItems:'center', gap:12, padding:14 }}>
                  ...
                  <View style={{ flex:1 }}>
                    ...
                    <Text style={{ fontSize:11, color:Colors.text3, marginTop:2 }}>
                      {new Date(m.originDate + 'T00:00:00').toLocaleDateString('en-US',
        ...
        {[
          ...
          <View key={i} style={{ flexDirection:'row', alignItems:'center', gap:14,
            ...
            borderColor:'rgba(62,207,178,0.18)', borderRadius:18,
